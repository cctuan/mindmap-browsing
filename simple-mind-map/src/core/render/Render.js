...
class Render {
  ...
  registerCommands() {
    ...
    this.mindMap.command.add('SET_NODE_REQUIRED_UPDATE', this.setNodeRequireUpdate.bind(this))
  }
  ...
  setNodeRequireUpdate(node, requireUpdate) {
    node.nodeData.data.requiredUpdate = !!requireUpdate
    console.log({requireUpdate})
    this.reRenderNodeCheckChange(node, true)
  }

  //  更新节点数据
  setNodeData(node, data) {
    if (data !== null && ['text', 'associativeLineParents'].some(key => data[key])) {
      node.children.forEach(childNode => {
        childNode.setNodeRequireUpdate(true)
      })
    }
